...
import type { ManualLogBleeding, TrackingGoal } from '@/src/types/database';
...
export type LocalBioScoreResult = {
  ...
  /** True when user is not premium — UI may upsell for full confidence range. */
  full_confidence_requires_premium?: boolean;
  /** True when free tier or limited-data cap was applied (70 direct / 30 floor for avoid raw). */
...
/**
 * Freemium / statistical-detective: cap confidence at 70% direct read, or avoid raw floor 30
 * (UI calm = 100 − raw, so calm never exceeds 70 when capped).
 */
...
function bleedingManualLogsFromSeries(seriesAsc: DailyFertilityInput[]): ManualLogs[] {
  ...
  for (const r of seriesAsc) {
    if (r.bleeding != null && String(r.bleeding).trim() !== '') {
      byDate.set(r.date, String(r.bleeding) as ManualLogBleeding);
    }
  }
...
function cycleDayFromCd1(cd1Iso: string, todayIso: string): number | null {
  ...
  while (cur < todayIso) {
    cd += 1;
    cur = isoDateString(addCalendarDays(parseIsoDate(cur), 1));
  }
  return cd;
}
...
/** Strict 3-over-6 on logged charted BBT (consecutive calendar days with temps); uses `thresholdDelta` unit. */
function strictSensiplanThreeOverSixLoggedBbt(
  ...
  for (let i = charted.length - 3; i >= 0; i -= 1) {
    ...
    const b = charted[i + 1]!;
    const c = charted[i + 2]!;
    const bExpect = isoDateString(addCalendarDays(parseIsoDate(a.date), 1));
    const cExpect = isoDateString(addCalendarDays(parseIsoDate(b.date), 1));
    if (b.date !== bExpect || c.date !== cExpect) continue;

    const priorTemps: number[] = [];
    let walk = isoDateString(addCalendarDays(parseIsoDate(a.date), -1));
    for (let guard = 0; guard < 400 && priorTemps.length < 6; guard += 1) {
      ...
      walk = isoDateString(addCalendarDays(parseIsoDate(walk), -1));
    }
...
function rollingBaselineAndLatest(
  ...
  const windowStartIso = isoDateString(addCalendarDays(parseIsoDate(currentIso), -14));
  const vals: number[] = [];
  for (const r of seriesAsc) {
...
/**
 * Avoid goal: raw pregnancy-risk score (higher = higher risk). UI continues to show `100 - score` as calm.
 * Calendar CD bands + HRV/RHR/RR early warning vs 14d baseline; BBT 3-over-6 closes window → raw 10.
 */
function calculateAvoidTunedLocalBioScore(
  ...
  const todayIso =
    seriesAsc.length > 0 ? seriesAsc[seriesAsc.length - 1]!.date : isoDateString(new Date());
 ...
 * Always caps free-tier output at {@link FREE_TIER_LOCAL_SCORE_CAP} (except avoid uses its own cap at 100).
 */
